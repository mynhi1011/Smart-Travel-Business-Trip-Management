...
- **Business Rule liên quan:** `BR-TR-01`, `BR-TR-08`
- **Acceptance Criteria:**
  ...
  - **AC 3.2 (Kiểm soát chi phí lưu trú khi sửa lịch trình):**  
    ...
    *When* Employee nhập chi phí khách sạn và phụ cấp công tác  
    *Then* Hệ thống cộng tổng chi phí lưu trú và phụ cấp, so sánh với tổng hạn mức tương ứng theo `BR-TR-08`; chỉ hiển thị một cảnh báo tổng hợp nếu tổng vượt hạn mức, không yêu cầu nhập lý do.
...
- **Business Rule liên quan:** `BR-TR-08`, `BR-TR-03`, `BR-TR-04`
- **Acceptance Criteria:**
  - **AC 4.1 (Kiểm tra hợp lệ - Không vi phạm):**  
    *Given* Tổng chi phí lưu trú và phụ cấp không vượt tổng hạn mức theo `BR-TR-08`, đồng thời đáp ứng thời hạn gửi theo `BR-TR-03`  
  ...
  - **AC 4.2 (Phát hiện vi phạm chính sách):**  
    *Given* Trip Request có tổng chi phí lưu trú và phụ cấp vượt tổng hạn mức theo `BR-TR-08`, hoặc vi phạm thời hạn gửi theo `BR-TR-03`  
    *When* Hệ thống thực hiện Policy Check  
    *Then* Giao diện hiển thị cảnh báo tổng hợp cho phần chi phí vượt hạn mức (không yêu cầu lý do); các điều kiện định tuyến duyệt Cấp 2 được xác định riêng theo `BR-TR-04`.
...
- **Acceptance Criteria:**
  ...
  - **AC 10.2 (Xuất báo cáo PDF chuyến công tác):**  
    *Given* Chuyến đi ở một trong các trạng thái `APPROVED`, `ONGOING`, `EXPENSE_SUBMITTED`, `EXPENSE_APPROVED` hoặc `CLOSED`, và người dùng hiện tại là Employee owner của Trip hoặc có role `FINANCE` (theo `D-12`)  
  ...
  - **AC 10.3 (Từ chối xuất PDF sai điều kiện):**  
    *Given* Người dùng không phải Employee owner của Trip và không có role `FINANCE` (VD: `ADMIN`/`TRAVEL_ADMIN`), hoặc Trip đang ở trạng thái khác 5 trạng thái cho phép ở AC 10.2  
    *When* Người dùng bấm nút "Export PDF Summary"  
    *Then* Hệ thống từ chối yêu cầu và không cho tải file PDF (theo `D-12`).
...
- **Business Rules Covered:** 8/8 (`BR-TR-01` đến `BR-TR-08`). Toàn bộ rules đều được cài cắm trực tiếp vào Acceptance Criteria.