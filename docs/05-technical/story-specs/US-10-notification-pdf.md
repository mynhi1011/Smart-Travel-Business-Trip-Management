...
**US-10B (PDF Export):** Người dùng xuất file PDF tóm tắt chuyến công tác khi có quyền (Employee owner hoặc FINANCE) và trip ở một trong các trạng thái `APPROVED`, `ONGOING`, `EXPENSE_SUBMITTED`, `EXPENSE_APPROVED`, `CLOSED` (D-12).
...
1. User là Employee owner hoặc FINANCE mở Trip Detail; trip thuộc một trong các trạng thái được phép theo D-12.
2. User bấm **"Xuất PDF"**.
