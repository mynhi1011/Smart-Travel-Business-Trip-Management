...
**US-10B (PDF Export):** Owner hoặc FINANCE được xuất PDF khi Trip ở một trong các trạng thái `APPROVED`, `ONGOING`, `EXPENSE_SUBMITTED`, `EXPENSE_APPROVED` hoặc `CLOSED`; các trạng thái khác không được export.
...
4. Server tạo PDF binary bằng bộ sinh PDF được cấu hình trong implementation (hiện controller sử dụng PDFKit); trả file dưới dạng attachment.
