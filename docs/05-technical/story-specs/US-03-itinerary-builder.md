...
## Quy tắc chỉnh sửa theo trạng thái (quyết định BA)

Trong MVP, owner được phép thêm, sửa và xóa itinerary ở mọi trạng thái Trip ngoại trừ `CLOSED`. Backend là nơi enforce quyền sở hữu và trạng thái; UI phải ẩn/disable các thao tác khi Trip đã `CLOSED`. Quyết định này thay thế câu hỏi “Tuỳ thiết kế” tại E-05.
...
| E-05 | Trip ở trạng thái bất kỳ ngoài `DRAFT` (ví dụ `SUBMITTED`, `APPROVED`, `ONGOING`, `EXPENSE_SUBMITTED`, `EXPENSE_APPROVED`) | Cho phép owner thêm/sửa/xóa itinerary nếu Trip chưa `CLOSED`; chỉ `CLOSED` bị chặn với `409 TRIP_IMMUTABLE`. Đây là hành vi MVP và phải được enforce ở server. |
...
> **Trạng thái câu hỏi:**
> 1. **Đã chốt – D-11:** Owner được sửa itinerary ở mọi trạng thái trừ `CLOSED`; xem quy tắc chỉnh sửa theo trạng thái ở trên.
> 2. **Chờ BE xác nhận (FIX-04):** Cơ chế `justification` khi hotel cost vượt trần cần được chốt và enforce ở backend; không được xem là quyết định đã hoàn tất trong US-03 cho đến khi API, persistence và UI được đồng bộ.
