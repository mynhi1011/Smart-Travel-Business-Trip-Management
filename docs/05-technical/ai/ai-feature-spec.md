...
`docs/TRACEABILITY.md` đang rỗng nên không cung cấp mapping bổ sung. Chi tiết implementation canonical được lấy từ requirements, API contract/OpenAPI, architecture, story specs, Vault và decision log. Decision D-09 đã chốt rằng `APPROVED_MANAGER` là legacy state và không được dùng trong API/UI. Các workflow/requirements còn chứa tham chiếu này cần được cập nhật để dùng chu kỳ trạng thái canonical; không xem đây là trạng thái hợp lệ của OpenAPI.
