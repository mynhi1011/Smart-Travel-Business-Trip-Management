...
|---|---|---|---|---|
| **BR-TR-01** | Hạn mức lưu trú theo Cấp bậc (Accommodation Limit) | Hạn mức tham chiếu chi phí phòng khách sạn tối đa cho 1 đêm: **Staff/Specialist:** 1.000.000 VNĐ; **Manager/Lead:** 1.800.000 VNĐ; **Director/Executive:** 3.000.000 VNĐ. | Không phát sinh cảnh báo hoặc yêu cầu nhập lý do riêng theo quy tắc này. Hạn mức được sử dụng để tính tổng hạn mức kết hợp tại BR-TR-08. | Decision **D-06** |
| **BR-TR-02** | Phụ cấp công tác phí hàng ngày (Per Diem Allowance) | Mức phụ cấp tham chiếu: **Đô thị loại 1** (Hà Nội, TP.HCM, Đà Nẵng): 400.000 VNĐ/ngày; **các tỉnh thành khác:** 300.000 VNĐ/ngày. `Max_Per_Diem = Số ngày × Mức khoán`. | Không phát sinh cảnh báo hoặc yêu cầu nhập lý do riêng theo quy tắc này. Mức phụ cấp được sử dụng để tính tổng hạn mức kết hợp tại BR-TR-08. | Travel Policy Standard |
...
| **BR-TR-05** | Ngưỡng cho phép vượt chi phí quyết toán (Expense Variance Tolerance) | Khi nộp Expense Claim sau chuyến đi: <br>• Nếu `Tổng chi thực tế ≤ Tổng dự toán`: Finance duyệt bình thường. <br>• Nếu `Tổng chi thực tế > Tổng dự toán` nhưng trong mức **≤ 10%**: Cần Employee nhập giải trình ngắn. <br>• Nếu `Tổng chi thực tế vượt > 10%`: Cần Employee nhập giải trình ngắn. Bắt buộc chuyển lại cho Manager phê duyệt bổ sung phần chênh lệch trước khi Finance được phép bấm Close. | Chặn nút Approve của Finance nếu phát sinh vượt >10% chưa có chữ ký duyệt bổ sung của Manager. | Financial Governance |
...
| **BR-TR-08** | Kiểm tra tổng hạn mức lưu trú và phụ cấp công tác ([đặc tả riêng](BR-TR-08-combined-cost-limit.md)) | `Combined_Actual = (hotelCostPerNight × hotelNights) + perDiemBudget`; `Combined_Limit = (Hotel_Limit[jobGrade] × hotelNights) + (tripDays × Per_Diem_Rate[destinationType])`. Chỉ đánh giá khi dữ liệu chi phí tương ứng được cung cấp. | Nếu `Combined_Actual > Combined_Limit`, hệ thống hiển thị cảnh báo tổng hợp. Nếu bằng hoặc thấp hơn thì không cảnh báo. Không yêu cầu nhập lý do. Không phát sinh cảnh báo riêng theo BR-TR-01 hoặc BR-TR-02. | Quyết định nghiệp vụ cập nhật |
